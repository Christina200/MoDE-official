...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # print(f'cos: {cos.shape} | q: {q.shape}')
...
class ChameleonMLP(nn.Module):
    ...
    def forward(self, x, token_type_ids=None):
        if token_type_ids is None:
            ...
            # down_proj = self.down_proj(self.act_fn(self.gate_proj(x, modality_ids=token_type_ids)) * self.up_proj(x, modality_ids=token_type_ids), modality_ids=token_type_ids)
            gate_out = self.gate_proj(x, modality_ids=token_type_ids)
            gate_out = self.act_fn(gate_out)
            up_out = self.up_proj(x, modality_ids=token_type_ids)
            gate_out.mul_(up_out)
            down_proj = self.down_proj(gate_out, modality_ids=token_type_ids)
        return down_proj
...
class ChameleonSdpaAttention(ChameleonAttention):
    ...
    def forward(
        ...
        bsz, q_len, channel = hidden_states.size()
        ...
        if token_type_ids is None: 
            ...
            query_states = self.q_proj(hidden_states, modality_ids=token_type_ids)
            key_states = self.k_proj(hidden_states, modality_ids=token_type_ids)
            value_states = self.v_proj(hidden_states, modality_ids=token_type_ids)
        ...
        # print(f'value states: {value_states.shape} | query states: {query_states.shape} | key states: {key_states.shape}')
        cos, sin = self.rotary_emb(value_states, position_ids)
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        # self.vocabulary_mapping = ChameleonImageVocabularyMapping(config.vocabulary_map, new_merges = None if (not hasattr(config, 'new_merges')) else config.new_merges)
    ...
    def get_image_tokens(self, pixel_values: torch.FloatTensor):
        ...
        # print(f'before bpe length: {image_toks.shape}')
        # bpe_toks = self.vocabulary_mapping.convert_img2bpe(image_toks)
        # for x in bpe_toks:
        #     print(f'after bpe length: {len(x)}')
        # not sure about the role of the below line, removing for now
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            if isinstance(image_tokens, list):
                ...
                # print(f'before device and dtype: {input_ids.device} and {input_ids.dtype}')
                input_id = input_ids[0]
                # print(f'before input id: {input_ids.shape}')
                image_token_id_locations = torch.where(input_id == self.vocabulary_mapping.image_token_id)[0].tolist()
                image_token_id_locations = image_token_id_locations[::1024]
                end = 0
                new_input_id = []
                for j in range(len(image_token_id_locations)):
                    image_token = image_tokens[j].to(device, dtype)
                    new_input_id.append(input_id[end:image_token_id_locations[j]]) 
                    new_input_id.append(image_token)
                    end = image_token_id_locations[j] + 1024
                if end != len(input_id):
                    new_input_id.append(input_id[end:])

                input_ids = torch.cat(new_input_id, dim = -1).unsqueeze(dim=0)
                # print(f'after device and dtype: {input_ids.device} and {input_ids.dtype}')
                # print(f'after input_ids: {input_ids.shape}')
            else:
        ...
        sq_length = inputs_embeds.shape[1]
        cache_position = cache_position[:sq_length]

        if position_ids is None:
            position_ids = cache_position.unsqueeze(0)

        position_ids = position_ids[:,:sq_length]
        # print(position_ids)
    ...
    def _update_causal_mask(
        ...
        if using_static_cache:
            ...
            # )
            expected_target_length = past_seen_tokens + sequence_length + 1
            if isinstance(attention_mask, torch.Tensor):
                actual_target_length = attention_mask.shape[-1]
                if actual_target_length < expected_target_length:
                    # Pad the attention mask to include the prompt tokens.
                    pad_length = expected_target_length - actual_target_length
                    attention_mask = torch.nn.functional.pad(attention_mask, (pad_length, 0), value=1)
        ...
        **kwargs,
        ...
        Args:
            ...
            batch_size (`torch.Tensor`):
                Batch size.
        """
        # print(f"prepare 4D | target length: {target_length}")
        # # If attention_mask is None, create a default mask that covers the full target length.
        # if attention_mask is None:
        #     attention_mask = torch.ones(batch_size, target_length, device=device, dtype=torch.long)

        # # If the mask is 2D but its length is less than target_length, pad it.
        # if attention_mask.dim() == 2 and attention_mask.shape[-1] < target_length:
        #     pad_length = target_length - attention_mask.shape[-1]
        #     attention_mask = torch.nn.functional.pad(attention_mask, (0, pad_length), value=1)
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        ## modes = ['inference-text', 'train-image', 'train-text', 'train-all']
        self.mode = 'inference-text'
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        token_type_ids=None,
        # Extra keys carried by the MoDE collator: they are consumed by the distillation
        # trainer, not here, but the batch is passed through as-is.
        task_id=-1,
        img_gen_input_ids=None,
        img_gen_attention_mask=None,
        img_gen_token_type_ids=None,
        ...
        if self.mode == 'train-text' or self.mode == 'inference-text':
            image_tokens = [i for i in range(4,8196)] + [i for i in range(65536, len(logits[0][0]))]
            logits[:, :, image_tokens] = torch.finfo(logits.dtype).min
            
        if self.mode == 'train-image':
...
class MMoEChameleonForConditionalGeneration(ChameleonForConditionalGeneration):
    def _prepare_model_inputs(
        ...
        # Call the parent method from GenerationMixin / ChameleonForConditionalGeneration
        inputs_tensor, model_input_name, model_kwargs = super()._prepare_model_inputs(
            inputs, bos_token_id, model_kwargs
        )
        
        # If token_type_ids is in the original inputs (e.g. if inputs is a dict with "token_type_ids")
        # or already present in model_kwargs, ensure it stays in model_kwargs.
        if model_kwargs is None:
            model_kwargs = {}
        
        if isinstance(inputs, dict) and "token_type_ids" in inputs:
            model_kwargs["token_type_ids"] = inputs["token_type_ids"]
        elif "token_type_ids" in model_kwargs:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Call the parent method to prepare model_inputs.
        ...
        # Inject token_type_ids if not already present.
        # Here we assume that during generation only the new token(s) are processed,
        # so we set token_type_ids to be a tensor of shape [batch_size, seq_len] with all values equal to self.new_modality.
        new_modality = 0 if self.mode == 'inference-text' else 1
        if "token_type_ids" not in model_inputs:
            if "input_ids" in model_inputs:
                batch_size, seq_len = model_inputs["input_ids"].shape
                model_inputs["token_type_ids"] = torch.full(
                    (batch_size, seq_len),
                    new_modality,
                    dtype=torch.long,
                ...
                batch_size, seq_len, _ = model_inputs["inputs_embeds"].shape
                model_inputs["token_type_ids"] = torch.full(
                    (batch_size, seq_len),
                    new_modality,
                    dtype=torch.long,
